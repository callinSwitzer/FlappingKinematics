...
write.table(as.data.frame(summary(m7)$coef),"clipboard",sep="\t")
...
anova(m1, m2)
car::vif(m2) # better, but still concerning
summary(m2)
...
summary(m7) # final model for paper, b/c we're interested in load
car::vif(m7)

# copy to clipboard, to paste into excel
write.table(data.frame(summary(m7)$coef),"clipboard",sep="\t")

m8 <- update(m7, .~. - deltaLoad)
anova(m7, m8) 
summary(m8)

m9 <- update(m8, .~. - order_1)
anova(m7, m9) # p-value for order

summary(m8) 

### model diagnostics
par(mfrow = c(2,3))
plot(m7, which = 1:6) # no glaring violations
par(mfrow = c(1,1))
...
y <- residuals(m7, type = 'partial')[, "order_1"]
...
1. Order is associated with a change in frequency^2.  We find no evidence of non-linear relationships or interactions.
2. We find no significant affect of deltaload on deltaFrequency^2
3. Holding other variables constant, if the bee was loaded second, then they had a lower deltaFrequency^2 than if they were loaded first. 
...
mm1 <- lm(deltaMetR ~ deltaFrq2 + deltaArcL2 + 
            deltaLoad + size_pc1 + deltaLoad2, data = newDF)
car::vif(mm1)

# note that the overall p-value, p = 0.0508 may be due to a "masking effect" of non-significant terms
# or due to collinearity of delta load
# see: https://stats.stackexchange.com/questions/24720/significance-contradiction-in-linear-regression-significant-t-test-for-a-coeffi
summary(mm1)

mm2 <- update(mm1, .~. - deltaLoad2)
anova(mm1, mm2)
summary(mm2)

mm3 <- update(mm2, .~. - size_pc1)
anova(mm3, mm2) 
summary(mm3)

# note mm3 is the model that includes delta arcleng2

mm4 <- update(mm3, .~. - deltaArcL2 )
anova(mm4, mm3) # pval for arcleng2
summary(mm4)
...
# copy to clipboard, to paste into excel
write.table(data.frame(summary(mm7)$coef),"clipboard",sep="\t")
...
fig5a <- ggplot(newDF, aes(x= dLoad_nonCent, y = y)) + 
  geom_point(shape = 17) + 
  xlim(c(0.03, 0.12)) + 
  labs(x = "Δ load (g)", y = "Δ Metabolic Rate partial residual" ) + 
  #stat_smooth(method = 'lm', se = FALSE, color = "black", lty = 2, size = 0.5) + 
...
partialPreds_deltaFreq2 <- predict(lm(y ~ newDF$deltaFrq2))
summary(mm7)
...
write_csv(partialMetRateDF, 
          file.path(dataOut, "PartialResid_MetRate_Fig5a_5b.csv"))

fig5b <- ggplot(partialMetRateDF, aes(x= deltaFreq2, y = partialDeltaFreq2)) + 
  geom_point(shape = 17) + 
  xlim(c(-6*10^3, 13e3)) + 
  labs(x = "Δ frequency^2", y = "Δ Metabolic Rate partial residual" ) + 
...
1. Deltafreq^2 is associated with deltaMetR.
2. Holding deltafreq constant (not necessarily at 0), an increase in deltaload is not associated with a significant change in deltametabolicRate
