---
title: "Bumblebee Flapping Kinematics  _ raw analysis"
...
1.  First we see how $\Delta$load affects $\Delta$amplitude and $\Delta$frequency, accounting for bee size and treatment order
2.  We check to see how $\Delta$frequency and $\Delta$amplitude are associated with $\Delta$MetabolicRate, while accounting for $\Delta$load
...
- ArcL = (.75 $\cdotp$ wingLen) * (amp ($\pi$/180)) 	pi/180 converts to radians
...
newDF <- newDF %>%
  mutate(
     ...
     deltaAmp = amp_H - amp_L,
     deltaFreq = freq_H - freq_L,
...
## REFREF###
tt = ((newDF$M2_H + newDF$MF_H) - (newDF$M2_L + newDF$MF_L))/newDF$Mstarved * 50

plot(tt, newDF$deltaPercLoad)

all.equal(tt, newDF$deltaPercLoad)


df2p = newDF %>%
  mutate(df2p = ((freq2_H - freq2_L) * Mstarved) / ((M2_H + MF_H - M2_L - MF_L)*50)
         )



plot(newDF$deltaFreq2Perc, df2p$df2p)

all.equal(newDF$deltaFreq2Perc, df2p$df2p)

apl = newDF %>%
  mutate(apl = ((M2_H + MF_H + M2_L + MF_L - 4*Mstarved)*25/Mstarved)
         )


all.equal(newDF$avgPercLoad, apl$apl)

plot(newDF$avgPercLoad, apl$apl)

plot(I(newDF$deltaFreq2Perc/newDF$Mstarved) ~ I(newDF$avgPercLoad*newDF$Mstarved))
plot(I(newDF$deltaFreq2Perc) ~ I(newDF$avgPercLoad))
...
# but not winglen, since it's used to calculate arclen
# 
aa = prcomp(newDF[, c("Mstarved", "Itspan", "S", "wingLen")], center = TRUE, scale = TRUE)
aa = prcomp(newDF[, c("Mstarved", "Itspan")], center = TRUE, scale = TRUE)
summary(aa) # 1st pc explains ~94% of the variance in the three measurements of size



plot(newDF[, c("Mstarved", "Itspan")])
cor(newDF[, c("Mstarved", "Itspan")])^2
...
car::scatterplotMatrix(newDF[, c("Mstarved", "Itspan", "S",  "size_pc1")])
...
# refref: exploration area:

"When we investigated the change in metabolic rate between light and heavy loading trials, we found that both Δ load and Δ frequency2 were significant predictors (Δ load, p = 0.044; Δ frequency2, p = 0.0066; Fig. 5A, Fig 5C, Table S4).  However, we found no evidence that Δ arc length2 was associated with changes in metabolic rate (p = 0.48; Fig. 5B; Table S4). "



```{r}
plot(newDF$deltaAmp, newDF$deltaMetR)
plot(newDF$deltaFreq, newDF$deltaMetR)

plot(newDF$deltaFreq, newDF$deltaFrq2)
plot(newDF$deltaAmp, newDF$deltaArcL)



x1 = rnorm(1000, 180, 20) 
x2 = 2*x1 #+ rnorm(1000, sd = 0.1)
...
plot(newDF$freq_L, newDF$deltaFreq)


plot(newDF$deltaFreq, newDF$deltaFrq2)


# model delta met rate ~ bee size + freq + amp

car::scatterplotMatrix(data.frame(newDF$deltaFreq, newDF$deltaAmp, newDF$deltaLoad, newDF$deltaArcL2))

# "When we investigated the change in metabolic rate between light and heavy loading trials, we found that both Δ load and Δ frequency2 were significant predictors (Δ load, p = 0.044; Δ frequency2, p = 0.0066; Fig. 5A, Fig 5C, Table S4).  However, we found no evidence that Δ arc length2 was associated with changes in metabolic rate (p = 0.48; Fig. 5B; Table S4).  

mm1 = lm(deltaMetR ~ deltaFreq + deltaAmp + deltaLoad , data = newDF)
summary(mm1)

# nearly identical to previous results
# b/c deltaFreq ~ deltaFrq2 * constant
plot(newDF$deltaFreq, newDF$deltaFrq2)
cor(newDF$deltaFreq, newDF$deltaFrq2)^2
# deltaArcL2 is a function of delta amp and wing len
plot(newDF$deltaAmp, newDF$deltaArcL2/newDF$wingLen^2)

cor(newDF$deltaAmp, newDF$deltaArcL2)^2


plot(newDF$deltaAmp, newDF$wingLen)
plot(newDF$wingLen, newDF$deltaArcL)
plot(newDF$wingLen, newDF$arcL_L)
plot(newDF$wingLen, newDF$amp_L)
plot(newDF$wingLen, newDF$freq_L)




plot(newDF$freq_L, newDF$amp_L)

plot(newDF$deltaFreq, newDF$deltaAmp)

summary(lm(newDF$deltaFreq ~ newDF$deltaAmp))
```
...
# Investigate how Δ load, bee size, and treatment order affected changes in amplitude between light and heavy trials

# refref here

# refref: how about mixed models?



```{r}
plot(newDF$wingLen, newDF$deltaAmp)
plot(newDF$deltaLoad, newDF$deltaAmp)
plot(newDF$wingLen,  newDF$deltaLoad)


plot(newDF$wingLen,  newDF$deltaPercLoad)

plot(newDF$deltaPercLoad,  newDF$deltaAmp)

plot(newDF$deltaLoad, newDF$deltaPercLoad)


plot(newDF$deltaPercLoad, newDF$Itspan) # appears to be uncorrelated
plot(newDF$deltaLoad, newDF$Itspan)


plot(newDF$arcL_L, newDF$amp_L)
plot(newDF$arcL_H, newDF$amp_H)
plot(newDF$deltaArcL^2, newDF$deltaAmp^2)
...
### refref: LMER
m1 <- lmer(bdta$amp ~ I(scale(bdta$load)) + I(scale(bdta$Itspan)) + bdta$order + (1|bdta$BeeID))
summary(m1)

m2 <- lmer(bdta$amp ~ I(scale(bdta$load)) + I(scale(bdta$Itspan)) + (1|bdta$BeeID))
summary(m2)

m2 <- lmer(bdta$amp ~ I(scale(bdta$load)) + (1|bdta$BeeID))
summary(m2)
...
# fit full model -- use delta amplitude 
m1 <- lm(deltaAmp ~  (deltaLoad +  Itspan + order_1)^2, data = newDF)
...
m2 <- update(m1, .~. - Itspan:deltaLoad)
car::vif(m2) 
summary(m2)

m3 <- update(m2, .~. - Itspan:order_1)
summary(m3)
anova(m2, m3) 
...
anova(update(m5, .~. - wingLen),  m5) # p-value for size
...
y <- residuals(m5, type = 'partial')[, "deltaPercLoad"]

# plot partial residuals with base R plotting
plot(x = newDF$deltaPercLoad, y = y)

predictedLine_partialResid = predict(lm(y ~ newDF$deltaPercLoad))

# plot partial residuals with base R plotting
arcLenDF <- data.frame(deltaLoad = newDF$dLoad_nonCent, partialResid_deltaArcLen2 = y, 
                       predictedLine_partialResid)
plot(arcLenDF)
...
fig4a <- ggplot(newDF, aes(x= dLoad_nonCent, y = y)) + 
  geom_point(shape = 17) + 
  xlim(c(0.03, 0.12)) + 
  labs(x = "delta load (g)", y = "Δ amp partial residual" ) + 
...
```{r}
##### LMER
m1 <- lmer(freq ~ I(scale(load)^2) +I(scale(load)) + I(scale(Itspan)) + order + (1|BeeID) , data = bdta)
summary(m1)
plot(m1)
...
# fit full model
m1 <- lm(deltaFrq ~  (deltaLoad +  size_pc1 + order_1)^2 , data = newDF)
plot(deltaFrq2~deltaFreq,  data = newDF)

lmm = lm(deltaFrq2~deltaFreq, data = newDF)
lines(newDF$deltaFreq, predict(lmm))
...
```{R}
plot(deltaPercLoad ~ avgPercLoad, data = newDF)
