...
```{r}
raw_data <- read_csv(file.path(dataDir, "beeRespData_freqUpdated.csv"))
summary(raw_data)
```
...
# refref: new data, says treatment is not significant
...
mod2 <- lm(log(force) ~ avg_mass, data =  bdta[bdta$treatment == "L", ])
summary(mod2)
...
plot(bdta$metabolic_rate ~ bdta$perc_load)
points(predict(m33, re.form = NA) ~ bdta$perc_load, col= 'red')


plot(freq~amp, data = bdta[bdta$treatment == "H",])
